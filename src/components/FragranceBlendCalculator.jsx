  ...
  return (
    <div
      ...
      style={{
        ...
        borderTop: bold ? `1px solid ${COLORS.ink}` : "none",
        marginTop: bold ? "6px" : "0",
        paddingTop: bold ? "14px" : "12px",
      }}
    >
      <span className={`text-sm ${bold ? "font-semibold" : ""}`} style={{ color: bold ? COLORS.ink : COLORS.inkSoft }}>
        {label}
      </span>
      <span className="text-right">
        ...
        <span className="block font-mono text-xs mt-0.5" style={{ color: COLORS.inkSoft }}>
    ...
    <div className="w-full max-w-3xl mx-auto p-6 sm:p-8" style={{ backgroundColor: COLORS.paper, color: COLORS.ink }}>
      {selectedPerfume && (
        <div className="mb-6 px-4 py-3 border flex items-center justify-between" style={{ borderColor: COLORS.line, backgroundColor: COLORS.card }}>
          <div>
            <span className="text-xs font-semibold" style={{ color: COLORS.inkSoft }}>From search</span>
          ...
          <button type="button" onClick={onClearSelection} className="text-xs font-mono underline" style={{ color: COLORS.inkSoft }}>
            Clear
          </button>
        </div>
      )}
        ...
        <div className="p-6 border" style={{ backgroundColor: COLORS.card, borderColor: COLORS.line }}>
          ...
          <Field label="Fragrance name">
            <div className="relative">
              ...
              {suggestions.length > 0 && (
                <div className="border mt-1" style={{ borderColor: COLORS.line, backgroundColor: "#fff" }}>
                  {suggestions.map((s) => (
                    <button
                      key={s.id}
                      type="button"
                    ...
                    >
                      {s.name} <span style={{ color: COLORS.inkSoft }}>— {TIERS[s.tier].label}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <details className="mt-2 mb-5">
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              {Object.keys(TIERS).map((key) => (
                <div key={key}>
                  <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>{TIERS[key].label} (g/mL)</label>
                  <TextInput type="number" step="0.01" value={densities[key]}
                    onChange={(e) => setDensities({ ...densities, [key]: parseFloat(e.target.value) || 0 })} />
                </div>
              ))}
              <div>
                <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Ethanol 96% (g/mL)</label>
          ...
          <div className="pt-1 border-t" style={{ borderColor: COLORS.line }}>
            <h4 className="text-xs font-semibold mt-4 mb-3" style={{ color: COLORS.forestDeep }}>
              Personal log <span className="font-normal" style={{ color: COLORS.inkSoft }}>— synced to your Supabase project</span>
            </h4>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Oil type / supplier</label>
                <input list="oilTypeOptions" value={oilType} onChange={(e) => setOilType(e.target.value)}
                  className="w-full px-3 py-2 font-mono text-sm border" style={{ borderColor: COLORS.line, backgroundColor: "#fff" }} />
                <datalist id="oilTypeOptions">
                  {OIL_TYPE_OPTIONS.map((o) => <option key={o} value={o} />)}
                </datalist>
              </div>
              <div>
                <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Price per gram</label>
                <TextInput type="number" step="0.01" min="0" value={pricePerGram} onChange={(e) => setPricePerGram(e.target.value)} placeholder="0.00" />
              </div>
            </div>
            <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Notes</label>
            <textarea
        ...
        <div className="p-6 border" style={{ backgroundColor: COLORS.card, borderColor: COLORS.line }}>
          ...
          {result.oilCost !== null && (
            <div className="flex items-center justify-between py-2 mt-2 text-sm font-mono">
              <span style={{ color: COLORS.inkSoft }}>Oil cost</span>
              <span style={{ color: COLORS.ink }}>{round2(result.oilCost)} (at {round2(Number(pricePerGram))}/g)</span>
            </div>
          )}

          <p className="text-sm italic mt-4" style={{ color: COLORS.inkSoft }}>{tier.note}</p>

          <div className="mt-6 grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Blended by</label>
              <TextInput value={blendedBy} onChange={(e) => setBlendedBy(e.target.value)} />
            </div>
            <div>
              <label className="block text-[11px] mb-1" style={{ color: COLORS.inkSoft }}>Date</label>
